#!/usr/bin/env python3
"""Unsubscribe email from cloud transfer notifications.

Removes an email address from the notification subscriber list.
...
def main():
    parser = argparse.ArgumentParser(
        description="Unsubscribe email from cloud transfer notifications"
    )
    parser.add_argument(
        "--email",
        required=True,
        help="Email address to unsubscribe",
    )
    parser.add_argument(
        "--login",
        default="login.json",
    ...
    # Unsubscribe
    result = notify.unsubscribe(args.email)

    # Handle result
    if not result["success"]:
        display_error(
            operation="Cloud Notification Unsubscribe",
