#!/usr/bin/env python3
"""Enable cloud transfer notifications.

Activates notification delivery. Configuration must be setup first.
...
def main():
    parser = argparse.ArgumentParser(
        description="Enable cloud transfer notifications"
    )
    parser.add_argument(
        "--login",
        default="login.json",
    ...
    result = notify.enable()

    # Handle result
    if not result["success"]:
        display_error(
            operation="Cloud Notification Enable",
